...
public class PasswordEvaluator {
	...
	public static boolean foundOtherChar = false;       // The flag for other characters
	private static String inputLine = "";				// The input line
	private static char currentChar;					// The current character in the line
	private static int currentCharNdx;					// The index of the current character
	private static boolean running;						// The flag that specifies if the FSM is 
														// running
	private static int charCounter = 0;                 // Char counter
	private static final int MAX_PASSWORD_LENGTH = 16;  // Maximum password length
	...
	public static String evaluatePassword(String input) {
		...
		// Check if password exceeds max length before processing
        if (input.length() > MAX_PASSWORD_LENGTH)
            return "*** Error *** The password must not exceed " + MAX_PASSWORD_LENGTH + " characters.";
		...
		setCharCounter(0);					// Initialize character counter
		...
		while (running) {
			...
			setCharCounter(getCharCounter() + 1); // Increment character count on each iteration
			...
			} else if ("~`!@#$%^&*()_-+{}[]|:,.?/".indexOf(currentChar) >= 0) { // Updated special character set
				System.out.println("Special character found");
				foundSpecialChar = true;
			} else {
				System.out.println("Other character found"); // Handling other characters
                foundOtherChar = true;
			}
			if (currentCharNdx >= 7) {
		...
		if (foundOtherChar) // Error message for invalid characters
            errMessage += "Invalid characters detected; ";
	...
	public static void setCharCounter(int charCounter) {
		PasswordEvaluator.charCounter = charCounter;
	}
}